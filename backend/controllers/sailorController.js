  ...
  try {
    ...
    const dashboardData = {
      ...
      currentShip: activeShip && {
        ...
        arrivalTime:
          activeShip.arrivalDate?.toISOString() || activeShip.eta.toISOString(),
        createdBy:
          portAuthoritydetails.firstName + " " + portAuthoritydetails.lastName,
...
export const updateSailorProfile = async (req, res) => {
  ...
  try {
    ...
    if (!updatedSailor) {
      return res
        .status(404)
        .json({ success: false, message: "Sailor not found" });
    }
...
export const getAvailableShipments = async (req, res) => {
  try {
    ...
    const hasAlreadyApplied = await JobPost.exists({
      crewAssigned: sailor._id,
    });
